...
$pdf->SetXY(10, 30);
$pdf->SetFont('', 'B', 11);
$pdf->Cell(0, 5, 'Laporan Pembayaran Hutang', 0, 1, 'C');
$pdf->SetFont('', '', 9);
if ($type == 2) {
    $pdf->Cell(0, 5, 'Periode : ' . gantiTanggal2($startdate) . ' s/d ' . gantiTanggal2($enddate), 0, 1, 'C');
    if ($id > 0) {
        $sql = "SELECT hutang.*, pemasok.nama, pemasok.alamat, pemasok.kota, pemasok.kodepos 
            FROM hutang 
            JOIN pemasok 
            ON hutang.id_pemasok = pemasok.id_pemasok 
            WHERE pemasok.id_pemasok = $id
            AND hutang.tanggal_pembayaran between '$startdate' and '$enddate'               
